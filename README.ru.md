...
секций, CSV/TSV как сортируемая таблица, форматирование JSON/XML/NDJSON, TOML/INI/.env как
таблица Ключ/Значение, умная типографика для простого текста, статистика документа
(адаптированный под русский индекс Флеша).

**PDF** — встроенный постраничный просмотрщик (PDFium через Skia, без WebView): рендер
по ширине, ленивые/виртуализированные страницы, зум Ctrl+±; откат к системному просмотрщику,
если нативный движок недоступен.

**Изображения** — растровые (PNG/JPEG/WebP/BMP/GIF/ICO) и SVG прямо в приложении: вписывание
по окну, зум Ctrl+±; растр декодируется нативно через Skia, SVG рендерится векторно.

**Диаграммы** — ` ```mermaid `/` ```plantuml `/` ```dot ` (и любой другой тип Kroki) рендерятся
в предпросмотре через сервер **Kroki** (Mermaid → PNG, остальные → SVG). Opt-in, по умолчанию
выключено — текст диаграммы отправляется на указанный сервер (публичный или self-hosted);
включается в Настройки ▸ Раскладка.
...
| PDF | PDFtoImage 4.x (PDFium, нативно, без WebView) |
| Изображения | Avalonia/Skia (растр) · Avalonia.Svg.Skia (SVG) |
