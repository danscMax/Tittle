...
public static class EditorBehavior
{
    ...
    // TITTLE_NOTM disables TextMate entirely (RAM isolation measurement).
    private static readonly bool TextMateDisabled =
        Environment.GetEnvironmentVariable("TITTLE_NOTM") is not null;
    ...
    // Test-only seams (InternalsVisibleTo Tittle.Tests). Let a headless test inspect the
